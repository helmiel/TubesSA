...
function tambahBarang() { 
    ...
    if (isNaN(berat) || isNaN(profit)) { // Jika tidak memasukkan nilai berat dan profit barang 
...
function exhaustiveSearchKnapsack() {
    ...
    for (let i = 0; i < (1 << n); i++) { // 1 << n artinya adalah menggeser bit 1 ke kiri sebanyak n kali
        let beratTotal = 0;
        let profitTotal = 0;
        ...
        if (beratTotal <= kapasitas && (!solusiOptimal || profitTotal > solusiOptimal.profitTotal)) {
    ...
    daftarSolusi.sort((a, b) => {
        ...
        return a.temp.toString().localeCompare(b.temp.toString());
    });
...
function displayResults(daftarSolusi, kapasitas, solusiOptimal) {
    ...
    maxValueDiv.innerText = `Max Value: ${solusiOptimal ? solusiOptimal.profitTotal : 0}`;
    resultDiv.appendChild(maxValueDiv);

    const itemsTakenDiv = document.createElement('div');
    itemsTakenDiv.innerText = `Items Taken: ${solusiOptimal ? solusiOptimal.temp.join(', ') : 'None'}`;
