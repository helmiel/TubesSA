...
function dynamicProgrammingKnapsack() {
  ...
  const daftarSolusi = Array.from({ length: n + 1 }, () =>
    Array(W + 1).fill(0)
  );
  ...
  for (let i = 1; i <= n; i++) {
    for (let w = 0; w <= W; w++) {
      if (daftarBarang[i - 1].berat <= w) {
        daftarSolusi[i][w] = Math.max(
          daftarSolusi[i - 1][w],
          daftarSolusi[i - 1][w - daftarBarang[i - 1].berat] +
            daftarBarang[i - 1].profit
        );
      } else {
  ...
  // Tracing solusi optimal barang yang akan diambil dari array daftarBarang (rekonstruksi solusi)
  let w = W;
  ...
  document.getElementById(
    "max-value"
  ).innerText = `Profit maksimal pada knapsack: ${daftarSolusi[n][W]}`;
  document.getElementById(
    "items-taken"
  ).innerText = `Barang yang diambil: ${ambilBarang.reverse().join(", ")}`;
